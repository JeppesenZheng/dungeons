...
public class Player extends Entity implements Battleable, OverlapInterface {
    ...
    public void onOverlap(GameMap map, Entity entity) {
        if (entity instanceof Enemy) {
            ...
            map.startBattle(this, (Enemy) entity);
        }
    }
    ...
    public boolean pickUp(Entity item) {
        if (item instanceof Treasure || item instanceof SunStone) collectedTreasureCount++;
        if (item instanceof Key) {
            int keys = inventory.getEntities(Key.class).size();
            if (keys > 0) {
                return false;
            }
        }
    ...
    public void useWeapon(Game game) {
        inventory.weaponUse(game);
    }

    public void useSceptre(Game game, String entityId) {
        inventory.useSceptre(game, entityId);
    }
    ...
    public <T extends InventoryItem> T getInventoryItem(Class<T> itemType) {
